...
doc: # Builds the manpage.
	$(SCDOC) <cmd/sitredctl/doc/sitredctl.1.scd >sitredctl.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 sitredctl $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 sitredctl.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
...
clean: # Cleans cache files from tests and deletes any build output.
	$(RM) -f cover.out sitredctl sitredctl.1
